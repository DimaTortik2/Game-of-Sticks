...
export function move_5(
	...
	// --- ПЛАН А: Поиск идеального выигрышного хода ---
	// (Этот код ищет ход, который приведет к g-числу 0)
	...
	for (let i = 0; i < piles.length; i++) {
		...
		if (pileSize - 1 >= 0) {
			...
			if (grundyValues.get(tempPiles.join(',')) === 0) return tempPiles
		}
		for (let j = 1; j <= (pileSize - 1) / 2; j++) {
	...
	for (let i = 0; i < piles.length; i++) {
		...
		if (pileSize >= 3) {
			...
			if (grundyValues.get(tempPiles1.join(',')) === 0) return tempPiles1

			for (let j = 1; j <= (pileSize - 3) / 2; j++) {
				const tempPiles2 = [...basePiles, j, pileSize - 3 - j].sort(
					(a, b) => a - b
				)
				if (grundyValues.get(tempPiles2.join(',')) === 0) return tempPiles2
			}
		}
	}

	// --- ПЛАН Б: Если выигрышный ход не найден (позиция проигрышная) ---
	// Делаем любой минимально возможный ход, чтобы остаться в игре.
	if (piles.length > 0) {
		console.warn('AI is in a losing position. Making a simple fallback move.')
		const newPiles = [...piles]
		// Находим самую большую группу, чтобы ход был более "человечным"
		...
		newPiles[largestGroupIndex]-- // Берем 1 палочку
		return newPiles.filter(p => p > 0) // Возвращаем новую позицию
	}

	// Если мы дошли сюда, значит, палочек на поле нет. Возвращаем null.
	return null
}
